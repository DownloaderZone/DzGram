...
def autofunction_page(title: str, names):
    body = header(title)
    for name in names:
        body += f".. autofunction:: pyrogram.Client.{name}\n\n"
    return body
...
def generate_methods():
    ...
    for cat in sorted(p for p in (PKG / "methods").iterdir() if p.is_dir() and p.name != "__pycache__"):
        ...
        for py in sorted(cat.glob("*.py")):
            ...
            if not methods:
                continue
            stem = py.stem.replace("_", "-")
            write(base / cat.name / f"{stem}.rst", autofunction_page(py.stem, methods))
            pages.append(stem)
    ...
    for cat in sorted(p for p in (PKG / "types").iterdir() if p.is_dir() and p.name != "__pycache__"):
        ...
        for py in sorted(cat.glob("*.py")):
            ...
            write(
                ...
                autoclass_page(py.stem, f"pyrogram.types.{cat.name}.{py.stem}", classes),
            )
            pages.append(stem)
...
def generate_enums():
    ...
    for py in sorted((PKG / "enums").glob("*.py")):
        ...
        if not classes:
            continue
        stem = py.stem.replace("_", "-")
        write(base / f"{stem}.rst", autoclass_page(py.stem, f"pyrogram.enums.{py.stem}", classes))
        pages.append(stem)
...
def generate_handlers():
    ...
    for py in sorted((PKG / "handlers").glob("*.py")):
        ...
        write(
            base / f"{stem}.rst",
            autoclass_page(py.stem, f"pyrogram.handlers.{py.stem}", classes),
        )
        pages.append(stem)
...
def generate_misc():
    ...
    for py in sorted((PKG / "storage").glob("*.py")):
        ...
        write(
            ...
            autoclass_page(py.stem, f"pyrogram.storage.{py.stem}", classes),
        )
    ...
    for py in sorted((PKG / "session").glob("*.py")):
        ...
        write(
            ...
            autoclass_page(py.stem, f"pyrogram.session.{py.stem}", classes),
        )
